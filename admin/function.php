...
function isLogin()
{
    if (!isset($_SESSION['login_maedonor'])) {
        redirect('../logout.php');
    } elseif (!db_count('pengguna', ['id' => $_SESSION['login_maedonor']])) {
        redirect('../logout.php');
    }
}
...
function user()
{
    return db_find('pengguna', ['id' => $_SESSION['login_maedonor']]);
}

function must_role($role)
{
    if (user()->role != $role) {
        redirectBack();
    }
...
function count_month($bulan, $darah = null)
{
    if (isset($_GET['tahun'])) {
        $tahun = $_GET['tahun'];
    } else {
    ...
    if ($darah == null) {
        return db_query("SELECT * FROM `permohonan` WHERE MONTH(tanggal) = $bulan && YEAR(tanggal) = $tahun")->num_rows;
    } else {
        return db_query("SELECT * FROM `permohonan` WHERE MONTH(tanggal) = $bulan && YEAR(tanggal) = $tahun && golongan_darah = '$darah'")->num_rows;
    }
}
